...
| 2026-05-12 | v1.1 sharp-interface verification | Phase 3 smoke | $2.1208\times10^5$ | $6.77\times10^1$ | $4.88$ | 5 | 4.0 s | Superseded pre-Excel-nondim Case B smoke; retained only as historical checkpoint-write confirmation |
| 2026-05-12 | v1.1 sharp-interface verification | Phase 3 smoke | $2.0211\times10^5$ | $7.97\times10^1$ | $4.92$ | 5 | 1.5 s | Updated case scale $\Theta=(T-T_w)/(T_{int}-T_w)$, so $\Theta_{in}=2$, $\Theta_{solidus}=1$, $\Theta_w=0$; superseded by the 2026-05-15 Excel-nondim implementation |
| 2026-05-12 | v1.1 sharp-interface verification, no $\mathcal{L}_{r,xx}$ | Phase 3 smoke | $2.0211\times10^5$ | $7.97\times10^1$ | $4.92$ | 5 | 2.8 s | Removed the interface axial-curvature penalty; smoke run wrote `outputs/no_rint_smooth_smoke/loss_history.csv` with no `rint_smooth` column and saved `checkpoints/no_rint_smooth_smoke/latest.pth` |
| 2026-05-12 | v1.1 sharp-interface verification, hot inlet wall | Phase 3 smoke | $2.0206\times10^5$ | $7.97\times10^1$ | $4.92$ | 5 | 2.0 s | Set $L=10.25$ and wall target $\Theta_{dep}=\Theta_{in}$ for $0\le x\le0.25$, otherwise $\Theta_{dep}=\Theta_w$; smoke confirmed both Phase 1 BC/IC and full-physics paths |
| 2026-05-14 | v1.1 sharp-interface verification + interface no-slip | Phase 3 smoke | $2.0206\times10^5$ | $7.97\times10^1$ | $4.92$ | 5 | 1.8 s | Added $\mathcal{L}_{u,\Gamma}$, fixed best-checkpoint payload metadata, confirmed checkpoint config loading in postprocess/diagnose, and verified `interface_vel` logging |
| 2026-05-15 | v1.2 Case B Excel nondim | Phase 3 smoke | $8.0561\times10^5$ | $7.99\times10^1$ | $5.14$ | 5 | 2.9 s | Implemented $\tau=tU/L_{total}$ with diameter-based $Pe_D=14.329521$, $Re_D=1.53$, $\tau_{end}=0.780487804878$; 91 pytest tests passed and postprocess smoke completed |

---

## 3. Accuracy vs MATLAB CFD Reference

**Active reference case:** Case B with $Pe_D=14.329521$, $Re_D=1.53$, $Ste=0.06275$, and $\tau_{end}=8/10.25=0.780487804878$.
...
| 2026-05-02 | Phase-change reintroduction with full collocation resampling and bounded CUDA refinement | Interface-only resampling at iter 6000 produced fresh weighted loss $3.90\times10^2$ dominated by $RMS(R_{E,f})=15.7$ and $RMS(R_{E,dep})=11.9$; full-batch resampling at iter 12000 reduced fresh weighted loss to $6.89\times10^{-1}$ | Bounded refinement from iter 12000 to 50000 used weight-only resumes, larger batches ($N_\Omega=2048$, $N_\Gamma=1024$, $N_{BC}=512$, $N_{IC}=1024$), $r\ge0.01$, full resampling every 200 iters, and LR $1e-5\to1e-6$ then $5e-6\to5e-7$. Final fresh weighted loss was $3.32\times10^{-1}$ with $RMS(R_{E,f})=0.212$, $RMS(R_{E,dep})=0.127$, $RMS(R_{mom,x})=0.0718$, $RMS(R_{mom,r})=0.0644$, $RMS(R_{mass})=0.0447$, $RMS(R_{Stefan})=0.0239$ | Accepted as the current stable phase-change path. Phase change is correctly coupled and physically smooth; remaining error is mainly wall/inlet temperature anchoring and fluid-energy residual, not Stefan failure |
| _(pending)_ | — | — | — | — |
