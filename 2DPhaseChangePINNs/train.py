"""
Two-phase training driver for the 2D pipe-solidification PINN.

Default settings follow config.py:
  Phase 1: BC/IC pre-training with Adam
  Phase 2: full physics with Adam + cosine annealing
  Phase 3: optional full-physics L-BFGS refinement, disabled by default

Use --smoke for a tiny CPU-safe run that verifies the default Phase 1 + Phase 2
pipeline.
"""
...
from config import Config, cfg, config_from_dict
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional L-BFGS refinement iterations; default 0 keeps Phase 3 disabled.",
    )
    ...
    parser.add_argument(
        "--smoke",
        action="store_true",
        help="Run a tiny Phase 1 + Phase 2 verification job.",
    )
...
def apply_cli_overrides(base: Config, args: argparse.Namespace) -> Config:
    ...
    if args.smoke:
        ...
        run_cfg.training.phase3_iters = 0 if args.phase3_iters is None else args.phase3_iters
...
def cleanup_previous_best_checkpoints(run_cfg: Config, keep_name: str) -> None:
...
def clear_last_checkpoint(run_cfg: Config) -> None:
    path = checkpoint_path(run_cfg, run_cfg.training.last_checkpoint)
    try:
        path.unlink()
        print(f"[checkpoint] removed stale {path.name}", flush=True)
    except FileNotFoundError:
        pass


def final_training_iteration(run_cfg: Config) -> int:
    phase1_end = run_cfg.training.phase1_iters
    phase2_end = run_cfg.training.phase2_iters
    phase3_end = phase2_end + max(run_cfg.training.phase3_iters, 0)
    return max(phase1_end, phase2_end, phase3_end)


def save_last_checkpoint(model: PINNSolidification,
                         run_cfg: Config,
                         phase: str,
                         iteration: int,
                         best_loss: float,
                         ...
                         optimizer_name: str | None) -> Path | None:
    return save_checkpoint(
        checkpoint_path(run_cfg, run_cfg.training.last_checkpoint),
        model,
        run_cfg,
        phase,
        iteration,
...
def save_best_and_latest(model: PINNSolidification,
    ...
    if math.isfinite(total) and total < best_loss:
        ...
        if best_path is not None:
            best_loss = total
            cleanup_previous_best_checkpoints(run_cfg, best_path.name)
...
def plot_monitor_snapshot(model: PINNSolidification,
    ...
    tt = torch.full_like(rr, 0.5 * run_cfg.case.tau_end)
    ...
    with torch.no_grad():
        ...
        r_int = model(
            torch.zeros_like(x),
            x,
            torch.full_like(x, 0.5 * run_cfg.case.tau_end),
    ...
    axes[0].set_title("Interface at tau_mid")
    ...
    axes[1].set_title("Theta_f at tau_mid")
...
def build_target_stop(args: argparse.Namespace, run_cfg: Config) -> TargetStop | None:
    ...
    return TargetStop(
        ...
        tau_value=run_cfg.case.tau_end if args.target_tau is None else args.target_tau,
...
def interface_thickness_stats(model: PINNSolidification,
    ...
    tau = torch.full_like(x, target.tau_value)
    r = torch.zeros_like(x)
    with torch.no_grad():
        r_int = model(r, x, tau)["r_int"]
...
def check_target_stop(model: PINNSolidification,
    ...
    print(
        ...
        f"at tau={target.tau_value:g} ({target.mode} target {target.thickness:.4f})",
        flush=True,
    )
...
def run_phase1(model: PINNSolidification,
    ...
    for iteration in range(start_iter + 1, end_iter + 1):
        ...
        target_reached = check_target_stop(model, run_cfg, device, target, "phase1", iteration)
        loss_target_reached = log["total"] < run_cfg.training.loss_target
        if target_reached or loss_target_reached or iteration >= final_training_iteration(run_cfg):
...
def run_phase2(model: PINNSolidification,
    ...
    for iteration in range(first_iter, phase2_end + 1):
        ...
        target_reached = check_target_stop(model, run_cfg, device, target, "phase2", iteration)
        loss_target_reached = log["total"] < run_cfg.training.loss_target
        if target_reached or loss_target_reached or iteration >= final_training_iteration(run_cfg):
            save_last_checkpoint(
                model, run_cfg, "phase2", iteration, best_loss, log,
...
def run_phase3(model: PINNSolidification,
    ...
    for iteration in range(first_iter, phase3_end + 1):
        ...
        target_reached = check_target_stop(model, run_cfg, device, target, "phase3", iteration)
        loss_target_reached = log["total"] < run_cfg.training.loss_target
        if target_reached or loss_target_reached or iteration >= final_training_iteration(run_cfg):
            save_last_checkpoint(
                model, run_cfg, "phase3", iteration, best_loss, log,
...
def main() -> None:
    ...
    if run_cfg.training.phase3_iters == 0:
        print("[train] phase3 disabled; default run stops after phase2", flush=True)
    else:
        print(
            f"[train] phase3 enabled for {run_cfg.training.phase3_iters} iterations",
            flush=True,
        )
    print(
        f"[train] case={run_cfg.case.name} Ste={run_cfg.case.Ste:g} Pe_D={run_cfg.case.Pe_D:g} "
    ...
    if target is not None:
        print(
            ...
            f"at tau={target.tau_value:g}, min_iter={target.min_iter}",
            flush=True,
        )
