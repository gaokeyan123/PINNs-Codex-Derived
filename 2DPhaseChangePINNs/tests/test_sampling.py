...
def test_interior_bounds():
    ...
    assert pts["tau"].min() >= 0.0 and pts["tau"].max() <= cfg.case.tau_end
...
def test_ic_t_zero():
    pts = sample_ic(N, cfg)
    torch.testing.assert_close(pts["tau"].detach(), torch.zeros(N))
