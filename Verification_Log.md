...
| 2026-05-12 | v1.1 sharp-interface verification | Phase 3 smoke | $2.1208\times10^5$ | $6.77\times10^1$ | $4.88$ | 5 | 4.0 s | Case `20260410_nonDimm_goodmatchCaseC`; Pe=14.3, Re=1.53, L=10, t_end=0.8, $\Theta_{in}=1$, $\Theta_{solidus}=0.5$, $\Theta_w=0$; code-path smoke only and `latest.pth` save confirmed |
| 2026-05-12 | v1.1 sharp-interface verification | Phase 3 smoke | $2.0211\times10^5$ | $7.97\times10^1$ | $4.92$ | 5 | 1.5 s | Updated case scale $\Theta=(T-T_w)/(T_{int}-T_w)$, so $\Theta_{in}=2$, $\Theta_{solidus}=1$, $\Theta_w=0$; `thickness_profiles.png` generation confirmed for $t=0.2,0.4,0.6,0.8$ |
